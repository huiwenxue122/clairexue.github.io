...
export default function GithubRepoCard({ repo, theme, coverImage }) {
  function openRepoinNewTab(url) {
    var win = window.open(url, "_blank");
    win.focus();
  }

  const withCover = Boolean(coverImage);
  ...
  return (
    <div
      className={`repo-card-div glass-repo-card${
        withCover ? " repo-card-div--with-cover" : ""
      }`}
    >
        ...
        >
          {withCover ? (
            <>
              ...
              <div className="repo-card-body">{titleAndDescription}</div>
            </>
          ) : (
            titleAndDescription
          )}
